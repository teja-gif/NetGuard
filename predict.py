...
from scapy.all import sniff, IP, TCP, UDP, ICMP, Raw, sr1
...
def calculate_and_process_flow(flow, end_time):
    features = {
        'duration': end_time - flow['start_time'],
        ...
        'wrong_fragment': 0,
        'urgent': flow.get('urgent_packets', 0)
    }
    ...
    if len(prediction_buffer) == 5:
        ...
        try:
            prediction, confidence = predict_attack_type(np.reshape(sequence, (1, 5, len(MODEL_COLUMNS))))
        except Exception:
...
def check_for_timeouts():
    while True:
        ...
        with flow_lock:
            ...
            for k in [k for k, f in list(flows.items()) if time.time() - f['last_seen'] > timeouts.get(f['protocol'], 60)]:
                if k in flows:
...
def calculate_and_emit_metrics():
    while True:
        ...
        with metrics_lock:
            ...
            total_bytes_sec, total_packets_sec, error_packets_sec = 0, 0, 0
            # Clean up old ICMP requests
            for key, ts in list(icmp_requests.items()):
                if time.time() - ts > 5:
                    del icmp_requests[key]
...
def process_packet(packet):
    with metrics_lock:
        ...
        # ICMP handling: measure latency via echo request/response
        if packet.haslayer(ICMP):
            try:
                # Echo Request
                if packet[ICMP].type == 8:
                    icmp_requests[(packet[IP].src, packet[IP].dst, packet[ICMP].id, packet[ICMP].seq)] = packet.time
                # Echo Reply
                elif packet[ICMP].type == 0:
                    key = (packet[IP].dst, packet[IP].src, packet[ICMP].id, packet[ICMP].seq)
                    if key in icmp_requests:
                        rtt_ms = (packet.time - icmp_requests.pop(key)) * 1000
                        latency_ms.append(rtt_ms)
                # Destination unreachable or errors
                if packet[ICMP].type == 3:
                    error_packets_sec += 1
            except Exception:
                pass

        # TCP-level errors (RST) increment error counter
        if packet.haslayer(TCP):
            try:
                # 'packet[TCP].flags' can be read reliably as a string representation
    ...
    # If an HTTP-like packet, hand it to the WAF worker via queue
    if packet.haslayer(TCP) and packet.haslayer(Raw) and (packet[TCP].dport == 80 or packet[TCP].sport == 80):
    ...
    with flow_lock:
        ...
        if key not in flows:
            ...
            if packet.haslayer(TCP) or packet.haslayer(UDP):
                flow.update({'src_port': packet.sport, 'dst_port': packet.dport})
            if flow['protocol'] == 'tcp':
                flow.update({'tcp_flags': set(), 'urgent_packets': 0, 'service': packet[TCP].dport})
            elif flow['protocol'] == 'udp':
                flow.update({'flag': 'SF', 'service': packet[UDP].dport})
            elif flow['protocol'] == 'icmp':
                flow.update({'flag': 'SF', 'service': ICMP_SERVICE_MAP.get((packet[ICMP].type, packet[ICMP].code), 'OTH')})
            flows[key] = flow

        flow = flows[key]
        flow['last_seen'] = current_time
        # add bytes to src or dst depending on packet direction
        try:
            # len(packet.payload) was used originally; keep similar behavior
            payload_len = len(packet.payload) if hasattr(packet, 'payload') else len(packet)
        except Exception:
        ...
        # --- TCP flag parsing FIX ---
        if flow['protocol'] == 'tcp' and packet.haslayer(TCP):
            try:
                flags_str = str(packet[TCP].flags)  # SAFE string form ('S', 'SA', 'R', etc.)
                flow['tcp_flags'].add(flags_str)

                if 'U' in flags_str:
                    flow['urgent_packets'] += 1

                if all(f in flags_str for f in ['S', 'F', 'A']):  # simultaneous S,F,A
                    flow['flag'] = 'SF'
                elif 'R' in flags_str:
                    flow['flag'] = 'REJ'
                elif 'S' in flags_str and len(flow['tcp_flags']) == 1:
                    flow['flag'] = 'S0'
            except Exception:
                pass

# Serve the dashboard HTML (index.html)
@app.route('/')
def index():
    try:
...
# ---------------------------
# Latency generator (new)
# ---------------------------
def latency_generator():
    """Continuously send ICMP echo requests to generate live latency."""
    target = "8.8.8.8"   # Google DNS (stable)
    while True:
        try:
            start = time.time()
            # send 1 ICMP echo request and wait for a reply (timeout=1s)
            reply = sr1(IP(dst=target)/ICMP(), timeout=1, verbose=False)
            if reply:
                rtt_ms = (time.time() - start) * 1000
                latency_ms.append(rtt_ms)
            else:
                # treat as a failed ping (append a large/zero value or skip)
                # Here we append 0 to indicate no response — dashboard will show effect
                latency_ms.append(0.0)
        except Exception:
            # ignore any exception from sending pings
            pass
        time.sleep(1)  # 1 ping per second
...
if __name__ == "__main__":
    if model:
        ...
        threading.Thread(target=latency_generator, daemon=True).start()  # <-- NEW latency generator thread
